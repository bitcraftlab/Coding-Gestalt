...
// there are six hamster data sets
int hamsters = 6;

// create a 2d array, for all data sets
int[][] data = new int[hamsters][];

// now load all the data sets into the data array
for(int i = 0; i < hamsters; i++) {
  data[i] =  int(trim(loadStrings("../../../data/circadian/hamster/HAMSTER" + (i + 1) + ".WHL")));
} 
 
// get number of datapoints 
int samples = data[0].length - 1;
...
int h = gap + (hamsters * days * pxlh) + hamsters * gap;
size(w, h);

// get min and max values for the activity
...
for(int i = 0; i < hamsters; i++) {
  for(int t = 0; t < samples; t++) {
    ...
    noStroke();
    fill(color(map(activity, amin, amax, 0, 255)));
    rect(x, y, pxlw, pxlh);
